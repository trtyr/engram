...
/// 项目**场景**模板（`projects.type` 值域 = 场景；预设分类建项目时复制进 `categories`，之后自由增删）。
///
/// 2026-09-21 用户拍定扩类：原先只有 dev/research 两值，实际「什么项目都往里放」——主机台账、
/// 部署中转这类运维/个人的事也被塞进 dev，分类失去意义。现按 6 场景分开（值域以迁移 0057 的
/// CHECK 为准，改类 = 此处常量 + 一条迁移）。
pub const PROJECT_TYPES: &[(&str, &[&str])] = &[
    ("dev", &["后端", "前端", "测试", "部署", "规划"]),
    ("ops", &["台账", "部署", "网络", "备份", "故障", "巡检"]),
    (
        "research",
        &["待查", "线索", "资料", "结论", "疑点", "证伪"],
    ),
    ("study", &["大纲", "笔记", "练习", "资源", "进度"]),
    ("life", &["计划", "清单", "记录", "花销", "参考"]),
