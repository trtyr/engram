...
async fn two_docs_interlinked_no_duplicate() {
    ...
    let pages = wiki
        .list_pages(lib, None, None, Some(50), None)
        .await
        .unwrap();
...
async fn list_pages_cursor_pagination_walks_all() {
    ...
    loop {
        let page = wiki
            .list_pages(lib, None, None, Some(2), cursor.as_deref())
            .await
            .unwrap();
        assert!(page.len() <= 2);
        if page.is_empty() {
    ...
    // 垃圾游标响亮拒
    let err = wiki
        .list_pages(lib, None, None, Some(2), Some("garbage"))
        .await
        .expect_err("垃圾游标应被拒");
...
async fn log_page_excluded_from_graph_and_lint_and_empty_ingest_rejected() {
    ...
    assert_eq!(
        svc.list_pages(lib, None, None, Some(100), None)
            .await
            .unwrap()
            .len(),
        1
