...
| [api.md](api.md) | 全部 80 路径 / 98 方法注册（当日 OpenAPI 活体导出）+ 分权矩阵 | 对接前端/写客户端 |
| [data-model.md](data-model.md) | 24 张业务表、25 个迁移、数据流 | 改 schema 前必读 |
...
cargo test --workspace        # 158 passed / 0 failed（36 个套件）
cargo run -q -p agent-memory-api --bin openapi-dump   # OpenAPI 导出：80 路径 / 98 方法
```
