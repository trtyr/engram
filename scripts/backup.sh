#!/usr/bin/env bash
# engram 备份/恢复。
# 备份:  ./backup.sh backup [输出目录]     → engram-<date>.tar.gz（pg_dump + data 卷）
...
case "$cmd" in
  backup)
    ...
    docker compose -f "$COMPOSE_FILE" exec -T db \
      pg_dump -U "${POSTGRES_USER:-agent}" -d "${POSTGRES_DB:-engram}" > "$tmp/db.sql"
    echo "== 打包数据卷"
    docker compose -f "$COMPOSE_FILE" exec -T app tar czf - -C /app data > "$tmp/data.tar.gz" 2>/dev/null || \
      docker run --rm -v engram_appdata:/data alpine tar czf - -C / data > "$tmp/data.tar.gz"
  ...
  restore)
    ...
    tar xzf "$f" -C "$tmp"
    echo "== 恢复数据卷"
    docker run --rm -v engram_appdata:/data -v "$tmp/data.tar.gz":/b.tar.gz alpine \
    ...
    docker compose -f "$COMPOSE_FILE" exec -T db \
      psql -U "${POSTGRES_USER:-agent}" -d "${POSTGRES_DB:-engram}" -c 'DROP SCHEMA public CASCADE; CREATE SCHEMA public;' >/dev/null
    docker compose -f "$COMPOSE_FILE" exec -T db \
      psql -U "${POSTGRES_USER:-agent}" -d "${POSTGRES_DB:-engram}" < "$tmp/db.sql" >/dev/null
