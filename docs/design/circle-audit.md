...
| 项 | 现状 | 建议 | 拍板点 |
|---|---|---|---|
| 实体关系升级 | 共现边 | 类型化关系表（关系类型 + 方向），或折中「共现 + 关系摘要」 | 是否建新表、关系类型枚举、谁写关系（蒸馏抽取 vs 手动） |
| 时间轴视图 | 无 | 按时间浏览实体/记忆的独立视图 | 视图形态（实体时间线 vs 全局记忆时间轴） |
| 批量操作 | 无 | 批量 merge/delete/归档 | 破坏性批量需 erase 分权 + 二次确认 |
| 实体导出 | 嵌 /memory/export | 圈子独立实体导出（csv/json） | 形态与隐私口径 |
