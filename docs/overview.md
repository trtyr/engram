...
三条旁路资产线：**LLM Wiki**（含 Knowledge 文档知识：文档→分块→向量检索 + 多源摄取→生成→图谱）、
**CodeGraph**（代码库结构索引）、**项目记忆**（跨会话工作线）。横切：任务队列（死信/重试/事件）、LLM 网关（多 provider
...
1. **人**——Engram 控制台（web/，八页 SPA：概览/用户记忆/圈子/Wiki/代码图谱/项目/任务/设置，墨白双主题；
