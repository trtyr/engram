...
> 最近更新 2026-09-08（**Wiki 真多库**：库切换器/库管理/全端点 ?lib= 与 MCP library 参数 / MCP 七工具 63 操作（+跨域 search_all）/ 会话批量恢复与擦除 / 单会话恢复 / Wiki 版本回滚与原料清理 / 字体定版 / 122 路径 / 37 迁移）。**三层结构**——本目录是全栈集成视角；
> 后端与前端各有独立项目档案。
...
| [api.md](api.md) | 89 路径域速览 + 分权矩阵 + 契约管理 | 找端点入口（全表在 server 档案） |
...
| [current-state.md](current-state.md) | 当日验证矩阵、Wiki 目录树大事记、运行环境实况、开放项 | **接手第一步** |
