...
全栈共一个 HTTP API，**67 路径 / 84 方法注册**（GET 34 · POST 37 · PUT 4 · PATCH 3 · DELETE 6，2026-09-01 运行中服务
...
- **erase scope**：会话擦除与 deep 清空需独立 scope（不可逆操作与读写分权）
- **deep purge 两阶段**：arm（5 分钟冷却）→ token 执行 / cancel 后悔药，job 行即审计链
