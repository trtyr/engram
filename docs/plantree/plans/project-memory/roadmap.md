...
2. **context pack 接口**：AI 开工拉上下文 / 收工沉淀协议（open-questions 6）。
3. **knowledge 检索打通**：project_docs 是否接 knowledge 全文检索管线（open-questions 1）。
4. **调研类型分类用户确认**：六分类草案已实现，实施前需用户确认（open-questions 7）。
