...
| Phase | 名称 | 依赖 | 状态 | 明细 |
|---|---|---|---|---|
| 0 | 项目地基 | — | **Done** (2026-08) | [phases/phase-0-foundation.md](phases/phase-0-foundation.md) |
| 1 | 核心底座 | 0 | **Done** (2026-08) | [phases/phase-1-core-infra.md](phases/phase-1-core-infra.md) |
| 2 | 记忆域（Chat Memory L0–L3） | 1 | **Done** (2026-08) | [phases/phase-2-memory.md](phases/phase-2-memory.md) |
| 3 | 知识域（Knowledge） | 1 | **Done** (2026-08) | [phases/phase-3-knowledge.md](phases/phase-3-knowledge.md) |
| 4 | Wiki 域 | 1 | **Done** (2026-08) | [phases/phase-4-wiki.md](phases/phase-4-wiki.md) |
| 5 | CodeGraph 桥 | 1 | **Done** (2026-08) | [phases/phase-5-codegraph.md](phases/phase-5-codegraph.md) |
| 6 | Web 控制台 | 2–5 | **Done** (2026-08) | [phases/phase-6-web.md](phases/phase-6-web.md) |
| 7 | 交付打磨与发布 | 6 | **Done** (2026-08) | [phases/phase-7-release.md](phases/phase-7-release.md) |

注：2/3/4/5 相互独立，可乱序或穿插执行；6 依赖各域 API 稳定。

## Done

- **Phase 7 交付打磨与发布**（2026-08）：干净环境 compose 全栈（重建修 3 个交付 bug：
...
- **Phase 6 Web 控制台**（2026-08）：七域完整 UI（Dashboard/Memory/Knowledge/Wiki/
...
- **Phase 5 CodeGraph 桥**（2026-08）：cg-bridge crate（CLI 子进程包装 + 版本 pin 1.5.0 +
  超时矩阵 + JSON 归一）+ 项目生命周期（注册/索引/同步/版本不匹配标记）+
  /codegraph/* 6 端点。真实仓库四类查询全通；修复测试容器泄漏根因（Drop 守卫），
  见 [evidence/README.md](evidence/README.md)
- **Phase 4 Wiki 域**（2026-08）：两步 ingest（analysis→generation，提示词版本化）+
...
- **Phase 3 知识域**（2026-08）：摄取管道三步链（parse→chunk→embed）+ SSRF 安全抓取
...
- **Phase 2 记忆域**（2026-08）：search（jieba 预分词 + pgvector + RRF 融合）+
...
- **Phase 1 核心底座**（2026-08）：10 份迁移（16 表全域 schema 定版）+ jobs 任务系统
...
- **Phase 0 项目地基**（2026-08）：9-crate workspace、Vite+React+shadcn 前端壳、
